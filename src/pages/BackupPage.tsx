...
const labels:Record<string,string>={products:'สินค้า',allowanceImports:'ประวัตินำเข้า Allowance',movementImports:'ไฟล์รายการเคลื่อนไหว',movementItems:'รายการเคลื่อนไหว',countSessions:'สาขาและรอบนับ',countSessionItems:'สินค้าในรอบนับ',countTransactions:'ประวัติการนับ',movementDrafts:'ฉบับร่างนำเข้า',auditUsers:'ผู้ใช้งาน',exportRecords:'ประวัติ Export'};
...
export default function BackupPage(){
  ...
  const restore=async()=>{if(!preview)return;try{setBusy(true);setError('');setConfirm(false);await restoreBackup(preview);setMessage('กู้คืนข้อมูลสำเร็จ กำลังโหลดข้อมูลใหม่');window.setTimeout(()=>window.location.reload(),700)}catch(reason){setError(reason instanceof Error?reason.message:'การกู้คืนล้มเหลว ข้อมูลเดิมยังคงอยู่');setBusy(false)}};
  return <Page title="สำรองและกู้คืนข้อมูล" subtitle="ดาวน์โหลดข้อมูลทั้งหมด หรือกู้คืนระบบจากไฟล์ Backup JSON">
    <div className="backup-grid">
      ...
      <section className="panel backup-card"><span className="backup-icon"><Upload/></span><div><h2 className="section-title">กู้คืนข้อมูล</h2><p>เลือกไฟล์ที่ระบบ Audit Stock ส่งออก ระบบจะตรวจสอบข้อมูลทั้งหมดก่อนเขียนทับ</p></div>
        ...
        {preview&&<div className="backup-preview"><h3>ตรวจพบไฟล์ Backup ที่รองรับ</h3><dl><div><dt>ชื่อไฟล์</dt><dd>{preview.fileName}</dd></div><div><dt>วันที่สำรอง</dt><dd>{preview.document.exportedAt?new Date(preview.document.exportedAt).toLocaleString('th-TH'):'ไม่ระบุ'}</dd></div><div><dt>เวอร์ชัน</dt><dd>{preview.document.version}</dd></div></dl><div className="backup-counts">{backupTableNames.map(name=><div key={name}><span>{labels[name]}</span><strong>{preview.counts[name].toLocaleString('th-TH')}</strong></div>)}</div><p className="backup-danger">การกู้คืนจะใช้ข้อมูลจากไฟล์แทนที่ข้อมูลปัจจุบัน</p></div>}
    ...
    <ConfirmDialog open={confirm} title="ยืนยันการกู้คืนข้อมูล?" detail="ระบบจะดาวน์โหลดสำรองข้อมูลปัจจุบันก่อน แล้วใช้ข้อมูลจากไฟล์แทนที่ข้อมูลทั้งหมด การดำเนินการนี้อาจใช้เวลาสักครู่" onCancel={()=>setConfirm(false)} onConfirm={()=>void restore()}/>
  </Page>
}
